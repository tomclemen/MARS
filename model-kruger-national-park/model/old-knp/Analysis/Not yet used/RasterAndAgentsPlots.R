...
# Connect to mongodb
ticks <- c(1,150,180) # will be passed to mongo query

collectionRasterLayer <- "f6ae7ad2-cfbf-4917-af79-263e102e2052-KNPGISRasterVegetationLayer"
conLayer <- mongo(collectionRasterLayer, url = "mongodb://127.0.0.1:27017/ResultData")

collectionAgents <- "f6ae7ad2-cfbf-4917-af79-263e102e2052-kf"
conAgents <- mongo(collectionAgents, url = "mongodb://127.0.0.1:27017/ResultData")


# output/input
outputRasterFilesAsGTiff <- FALSE
outputPngFilename <- "~/Downloads/griddata-mongo-%TICK%.png" # TICK placeholder will be replaced via regex
png( file=gsub("*%TICK%*",1,outputPngFilename), height=1754, width=1240, res=150 )
...
for (tick in ticks) {
  ...
  plots[[i]] <- ggplot() +
    ...
    scale_fill_gradient(low="darkseagreen1",high="springgreen4", breaks = bm_brk, name="Biomass UNIT") + 
    ...
    pointsPlot+
    guides(alpha=FALSE)+ #removing alpha legend
